...
describe('TCP agent over WebSocket (e2e)', () => {
  ...
  before(async () => {
    ...
    tlsTrustedPort = await findFreePort();
    tlsTrustedProc = spawnServer(tlsTrustedPort, {
      TCP_AGENT_ALLOWED_PORTS: '6379',
      TCP_AGENT_REQUIRE_TLS: 'true',
      TCP_AGENT_TRUSTED_PROXIES: '127.0.0.1',
    });
    await waitForListening(tlsTrustedProc);
  ...
  after(async () => {
    for (const proc of [serverProc, disabledProc, originProc, tlsProc, tlsTrustedProc, agentCredsProc]) {
      if (!proc) continue;
      proc.kill();
  ...
  it('enforces TLS when TCP_AGENT_REQUIRE_TLS is set', async () => {
    ...
    const forged = await wsConnectUpgrade(`http://localhost:${tlsPort}/tcp`, basicAuth('admin', 'secret'), {
      'X-Forwarded-Proto': 'https',
    });
    assert.equal(forged.status, 426);
  ...
  it('accepts forwarded https from a trusted proxy', async () => {
    const forwarded = await wsConnectUpgrade(`http://localhost:${tlsTrustedPort}/tcp`, basicAuth('admin', 'secret'), {
      'X-Forwarded-Proto': 'https',
    });
