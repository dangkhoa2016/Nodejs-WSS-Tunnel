...
├── dist/
│   ├── client.js           # Standalone esbuild bundle for tunnel clients (46KB, no server deps)
│   └── tcp-agent.js        # Standalone esbuild bundle for TCP agents (27KB, no server deps)
├── docs/
...
│   ├── TunnelServer.js     # HTTP Server & WebSocketServer (ws) initialization, graceful shutdown
...
│   ├── VirtualSocket.js    # Server-side virtual socket bridging an agent and the tunnel client
...
│   └── logger.js           # Logging system (text / JSON, verbose)
