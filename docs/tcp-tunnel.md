...
4. The tunnel client opens a TCP connection to the local service (`TCP_TUNNEL_HOST:port`) — that is Redis — and confirms with a `TCP_OPEN_ACK` frame.
5. Only after that ACK does the server start relaying data (`TCP_CONNECT_ACK` to the agent in agent mode), closing the race where a rejected open would still carry data. Data then flows in both directions using `TCP_DATA`/`TCP_CLOSE`/`TCP_ABORT` frames.
...
| `TCP_OPEN_ACK` | `0x44` | Client confirms the local TCP connection was opened (server then ACKs the agent) |
...
Expected log output (each line is prefixed with an ISO timestamp):
...
`agentPath`/`allowedPorts` only appear when the TCP agent endpoint is enabled (`TCP_AGENT_ALLOWED_PORTS` non-empty). Pin `installUuid` in `.env` as `INSTALL_UUID=...` so artifact URLs stay stable across restarts.
...
kill $(cat ~/.tunnel-client/client.pid) # stop the client (verify the PID belongs to client.js first)
```
...
>
> **Which host does the tunnel client dial?** The server's `TCP_TUNNEL_HOST`
> (default `127.0.0.1`) is the value sent in `TCP_OPEN`, so the client dials
> `TCP_TUNNEL_HOST:<agent-port>` on the service machine. The client must allow
> that host in `TCP_CLIENT_ALLOWED_HOSTS` (its default is its own
> `TCP_TUNNEL_HOST`, so `127.0.0.1` works out of the box). If the server's
> `TCP_TUNNEL_HOST` is changed, update the client's `TCP_CLIENT_ALLOWED_HOSTS`
> to match — otherwise the client logs `[verbose] [tcp] open_reject ... reason=host_not_allowed`.
...
| Server prints `[config] SECURITY WARNING: TCP_TUNNEL_BIND_HOST=0.0.0.0 with no TCP_TUNNEL_ALLOWED_IPS set.` | Binding `0.0.0.0` with an empty allowlist — Redis is exposed to the whole network. Narrow `TCP_TUNNEL_ALLOWED_IPS`. |
...
| `[verbose] [tcp] reject reason=no_client` | No tunnel client is connected over WebSocket yet. Run `client.js`/`setup.sh` on the Redis machine. |
| `[standard] [tcp] reject reason=per_port_limit` | Over `TCP_MAX_CONNECTIONS_PER_PORT` (default 20). Increase it or set `0` (unlimited). |
| `[standard] [client] disconnected code=1013 reason="Too many tunnel clients"` | A client beyond the `MAX_TUNNEL_CLIENTS` limit was rejected. Increase `MAX_TUNNEL_CLIENTS` to allow more concurrent tunnel clients. |
| `[standard] [client] auth_failed` | Wrong `TUNNEL_USERNAME`/`TUNNEL_PASSWORD` on the client. |
| Client logs `[verbose] [tcp] connect_timeout` / `local_error ... ECONNREFUSED` | The client cannot dial `TCP_TUNNEL_HOST:port`. Verify Redis is running on port `6379`, and `TCP_CLIENT_ALLOWED_HOSTS` includes that host. |
| Client logs `[verbose] [tcp] open_reject ... reason=host_not_allowed` | `TCP_CLIENT_ALLOWED_HOSTS` does not include the host the client is dialing. |
