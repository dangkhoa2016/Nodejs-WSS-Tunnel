...
4. Tunnel client mở kết nối TCP tới dịch vụ local (`TCP_TUNNEL_HOST:port`) — chính là Redis — rồi chuyển tiếp dữ liệu hai chiều qua các frame `TCP_DATA`/`TCP_CLOSE`/`TCP_ABORT`.

### Đặc điểm quan trọng

- **Một client tại một thời điểm**: máy chủ chỉ chấp nhận **1** tunnel client (`MAX_TUNNEL_CLIENTS = 1`); client thừa bị từ chối với close code `1013`.
- **Port không được remap**: server gửi `port: serverPort` trong frame `TCP_OPEN` (`src/TcpRouter.js:123`), client dial đúng cổng đó tới `TCP_TUNNEL_HOST`. Nghĩa là dịch vụ local **phải listen đúng cổng** mà app ngoài nối tới (ví dụ Redis phải ở `6379`, không đổi được sang cổng khác).
...
> ⚠️ **Cảnh báo bảo mật**: nếu `TCP_TUNNEL_BIND_HOST=0.0.0.0` mà `TCP_TUNNEL_ALLOWED_IPS` để trống, server in cảnh báo `SECURITY WARNING` vì Redis sẽ bị phơi ra toàn mạng. Luôn hẹp `TCP_TUNNEL_ALLOWED_IPS` lại.
>
...
kill $(cat ~/.tunnel-client/client.pid) # dừng client
```
...
| Server in `SECURITY WARNING: TCP_TUNNEL_BIND_HOST=0.0.0.0 with no TCP_TUNNEL_ALLOWED_IPS set` | Đang bind `0.0.0.0` mà allowlist trống — Redis lộ ra toàn mạng. Hẹp `TCP_TUNNEL_ALLOWED_IPS` lại. |
| `[standard] tcp reject reason=ip_not_allowed` | IP của app ngoài không nằm trong `TCP_TUNNEL_ALLOWED_IPS`. Thêm IP/CIDR tương ứng. |
| `[standard] tcp reject reason=no_client` | Chưa có tunnel client nào kết nối WebSocket. Chạy `client.js`/`setup.sh` trên máy Redis. |
...
| `[client] Connection closed: 1013` | Client thứ hai bị từ chối vì server chỉ nhận 1 tunnel client. |
| `[client] Authentication failed. Check username/password.` | Sai `TUNNEL_USERNAME`/`TUNNEL_PASSWORD` phía client. |
| Client báo `TCP connect timeout` / `connect ECONNREFUSED` | Client không dial được `TCP_TUNNEL_HOST:port`. Kiểm tra Redis đang chạy, đúng cổng `6379`, và `TCP_CLIENT_ALLOWED_HOSTS` chứa host đó. |
| Client báo `Host ... not allowed` | `TCP_CLIENT_ALLOWED_HOSTS` không chứa host client đang dial. |
