...
test('RC-HYBRID-01 code-only rollback uses current config, not partial old', async (t) => {
  ...
  // install2 should exit non-zero (candidate failed, rollback may exit after cleanup)
  // The key assertion: rollback process exists and uses CURRENT config
  const pid2 = readTrim(path.join(workDir, 'client.pid'));
  assert.ok(pid2, 'PID file must exist after rollback');

  // Wait for rollback process to become ready
  for (let i = 0; i < 30; i++) {
    if (isRunning(pid2)) break;
    await new Promise((r) => setTimeout(r, 500));
  }
  assert.ok(isRunning(pid2), `rollback process must be running; pid=${pid2}`);

  // Ready file must match PID
  const readyContent = readTrim(path.join(workDir, 'client.ready'));
  assert.equal(readyContent, pid2, 'ready PID must match running PID');
...
test('INT-02 service: SIGTERM during staging leaves old process untouched', async (t) => {
  ...
  const mockBin = createMockBin(sandbox, path.join(sandbox, 'npm-calls.txt'));
  // P2-3: Use a delayed artifact server — wait for the request to arrive
  ...
  const delayServer = http.createServer((req, res) => {
    // Block the response indefinitely — the installer will be stuck downloading
    pendingRequests.push({ req, res });
    resolveRequestSeen();
  });
  const delayPort = await new Promise((resolve) => {
  ...
  // Step 2: Start installer — the bundle URL points to the delay server which
  // never responds, so the installer remains in the staging phase.
  const child = spawn('bash', [SCRIPT.service], {
    cwd: ROOT,
    env: serviceEnv(sandbox, server.port, {
      mockBin,
      TUNNEL_PASSWORD: 'new-password',
      CLIENT_BUNDLE_URL: `http://127.0.0.1:${delayPort}/slow-client.js`,
    }),
    stdio: ['pipe', 'pipe', 'pipe'],
  });

  // Step 3: Wait for the request to reach the delayed server, then SIGTERM
  await requestSeen;
  child.kill('SIGTERM');
...
// ---------------------------------------------------------------------------
// INT-04 — service host: A → B → C three-generation SIGTERM rollback
// ---------------------------------------------------------------------------

test('INT-04 service: A→B→C SIGTERM restores B and B process', async (t) => {
  const sandbox = fs.mkdtempSync('/tmp/int04-svc-');
  const mockBin = createMockBin(sandbox, path.join(sandbox, 'npm-calls.txt'));
  const server = await createArtifactServer({
    ...fixturesFor('client', READY_BUNDLE),
    '/slow-client.js': SLOW_READY_BUNDLE,
  ...
  // Step 1: Install A
  const installA = await runScript(
    SCRIPT.service,
    serviceEnv(sandbox, server.port, { mockBin, TUNNEL_PASSWORD: 'correct-password' }),
  );
  assert.equal(installA.status, 0, `installA stderr:\n${installA.stderr}`);
  const releaseA = fs.readlinkSync(path.join(workDir, 'current'));
  const pidA = readTrim(path.join(workDir, 'client.pid'));
  assert.ok(isRunning(pidA), 'release A must be running');

  // Step 2: Install B (successful upgrade)
  const installB = await runScript(
    SCRIPT.service,
    serviceEnv(sandbox, server.port, { mockBin, TUNNEL_PASSWORD: 'correct-password' }),
  );
  assert.equal(installB.status, 0, `installB stderr:\n${installB.stderr}`);
  const releaseB = fs.readlinkSync(path.join(workDir, 'current'));
  const pidB = readTrim(path.join(workDir, 'client.pid'));
  assert.ok(releaseB !== releaseA, 'release B must differ from A');
  assert.ok(isRunning(pidB), 'release B must be running');

  // Verify previous symlink points to A
  const prevLink = fs.readlinkSync(path.join(workDir, 'previous'));
  assert.equal(prevLink, releaseA, 'previous must be A after installing B');

  // Step 3: Start candidate C (slow, never ready)
  const child = spawn('bash', [SCRIPT.service], {
    cwd: ROOT,
    env: serviceEnv(sandbox, server.port, {
      mockBin,
  ...
  // Capture candidate C PID before sending SIGTERM
  const candidatePidC = readTrim(path.join(workDir, 'client.pid'));
  assert.ok(candidatePidC, 'candidate C PID must be captured before SIGTERM');

  // Step 5: SIGTERM the installer
  child.kill('SIGTERM');
  ...
  // Step 6: Assert rollback to B
  assert.notEqual(result.status, 0, `installer must exit nonzero; stderr:\n${result.stderr}`);

  // Current must be restored to B
  const currentLink = fs.readlinkSync(path.join(workDir, 'current'));
  assert.equal(currentLink, releaseB, `current must be B; got ${currentLink}`);

  // Previous must be restored to A
  const prevAfter = fs.readlinkSync(path.join(workDir, 'previous'));
  assert.equal(prevAfter, releaseA, `previous must be A; got ${prevAfter}`);

  // Running process must be B's release
  const pidC = readTrim(path.join(workDir, 'client.pid'));
  assert.ok(pidC, 'PID file must exist after rollback');
  for (let i = 0; i < 30; i++) {
    if (isRunning(pidC)) break;
    await new Promise((r) => setTimeout(r, 500));
  }
  assert.ok(isRunning(pidC), `restored process must be running; pid=${pidC}`);

  // P2-2: Candidate C must not survive interrupted rollback
...
// ---------------------------------------------------------------------------
// INT-05 — agent host: A → B → C three-generation SIGTERM rollback
// ---------------------------------------------------------------------------

test('INT-05 agent: A→B→C SIGTERM restores B and B process', async (t) => {
  const sandbox = fs.mkdtempSync('/tmp/int05-agent-');
  const mockBin = createMockBin(sandbox, path.join(sandbox, 'npm-calls.txt'));
  const server = await createArtifactServer({
    ...fixturesFor('tcp-agent', READY_BUNDLE),
    '/slow-agent.js': SLOW_READY_BUNDLE.replace(/TUNNEL_READY_FILE/g, 'AGENT_READY_FILE'),
  ...
  assert.equal(installA.status, 0, `installA stderr:\n${installA.stderr}`);
  const releaseA = fs.readlinkSync(path.join(workDir, 'current'));
  const pidA = readTrim(path.join(workDir, 'agent.pid'));
  assert.ok(isRunning(pidA), 'release A must be running');
  ...
  assert.equal(installB.status, 0, `installB stderr:\n${installB.stderr}`);
  const releaseB = fs.readlinkSync(path.join(workDir, 'current'));
  const pidB = readTrim(path.join(workDir, 'agent.pid'));
  assert.ok(releaseB !== releaseA, 'release B must differ from A');
  assert.ok(isRunning(pidB), 'release B must be running');
  ...
  // Capture candidate C PID before sending SIGTERM
  const candidatePidC = readTrim(path.join(workDir, 'agent.pid'));
  assert.ok(candidatePidC, 'candidate C PID must be captured before SIGTERM');
  ...
  const pidC = readTrim(path.join(workDir, 'agent.pid'));
  assert.ok(pidC, 'PID file must exist after rollback');
  for (let i = 0; i < 30; i++) {
    if (isRunning(pidC)) break;
    await new Promise((r) => setTimeout(r, 500));
