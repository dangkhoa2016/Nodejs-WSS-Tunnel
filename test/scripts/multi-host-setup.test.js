...
test('roles use the correct WebSocket paths and loopback binding', () => {
  assert.match(serviceHost, /TUNNEL_SERVER_URL=wss:\/\/\$SERVER_HOST\/tunnel/);
  assert.match(appHost, /TUNNEL_SERVER_URL=wss:\/\/\$SERVER_HOST\/tcp/);
  assert.match(appHost, /AGENT_BIND_HOST=\$AGENT_BIND_HOST/);
});
