...
Unit tests for utility functions in `src/shared/utils.js`:
...
Unit tests for the logging system in `src/shared/logger.js`:
...
Unit tests for the stream manager in `src/server/StreamManager.js`:
...
Unit tests for TCP stream lifecycle in `src/server/StreamManager.js`:
...
Unit tests for `src/tcp/TcpClientHandler.js`:
...
Unit tests for `src/tcp/TcpRouter.js`:
...
Unit tests for `src/shared/ipAllowlist.js`:
...
Unit tests for TCP flow control coordination in `src/tcp/TcpFlowControl.js`:
...
1. **Test Environment Isolation (`NODE_ENV=test`)**:
   When running tests, `src/shared/config.js` skips loading the local `.env` file to avoid overwriting the fixed port `7860`, allowing the Server in `config-endpoint.test.js` to listen on a random port allocated by the test runner.
